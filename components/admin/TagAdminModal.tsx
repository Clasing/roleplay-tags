...
type TabType = 'skills' | 'grammar' | 'vocabularies' | 'categories';
...
export default function TagAdminModal({ isOpen, onClose }: TagAdminModalProps) {
  ...
  const loadData = useCallback(async () => {
    const [skillsData, subSkillsData, grammarData, subGrammarData, languagesData, vocabulariesData, subVocabulariesData, categoriesData, subCategoriesData] = await Promise.all([
      getSkills(),
      getSubSkills(),
      getGrammarTypes(),
      getSubGrammarTypes(),
    ...
    setCategories(sortByValue(categoriesData));
    setSubCategories(sortByValue(subCategoriesData));
  }, []);
  ...
  const filteredSubCategories = subCategories.filter(sub => {
    const selectedCategoryId = categories.find(c => c.value === selectedCategory)?.id;
    const subCategoryParent = (sub as unknown as { category?: string }).category ?? sub.categoryId;
    const matchesCategory = !selectedCategoryId || subCategoryParent === selectedCategoryId;
    const matchesSearch = sub.value.toLowerCase().includes(searchSubCategory.toLowerCase());
  ...
  const handleAddCategory = async () => {
    ...
    const success = await createCategory(newCategoryValue.trim());
    if (success) {
      setNewCategoryValue('');
  ...
  const handleAddSubCategory = async () => {
    if (!newSubCategoryValue.trim() || !selectedCategory) return;
    const categoryId = categories.find(c => c.value === selectedCategory)?.id;
    if (!categoryId) return;

    const success = await createSubCategory(newSubCategoryValue.trim(), categoryId);
    if (success) {
  ...
  const handleUpdateCategory = async () => {
    if (!editingCategoryId || !editingCategoryValue.trim()) return;
    const trimmedValue = editingCategoryValue.trim();
    const success = await updateCategory(editingCategoryId, trimmedValue);
    if (success) {
      if (selectedCategory === editingCategoryOriginalValue) {
  ...
  const handleUpdateSubCategory = async () => {
    if (!editingSubCategoryId || !editingSubCategoryValue.trim()) return;
    const trimmedValue = editingSubCategoryValue.trim();
    const success = await updateSubCategory(editingSubCategoryId, trimmedValue);
    if (success) {
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        ...
        if (success) {
          ...
          pushToast('success', 'Category eliminada');
          await loadData();
        } else {
          pushToast('error', 'No se pudo eliminar la category');
        }
      }
    });
  };

  const handleDeleteSubCategory = async (id: string, value: string) => {
    setConfirmState({
      title: 'Eliminar sub category',
      message: `¿Seguro que quieres eliminar "${value}"?`,
      onConfirm: async () => {
        const success = await deleteSubCategory(id);
        if (success) {
          pushToast('success', 'Sub category eliminada');
          await loadData();
        } else {
          pushToast('error', 'No se pudo eliminar la sub category');
        }
      }
    });
  };
          ...
          <div className="border-b border-gray-200 bg-white px-4 dark:border-zinc-800 dark:bg-black sm:px-6 lg:px-8">
            <div className="flex gap-1">
              <button
                ...
                className={`group relative px-4 py-3 text-sm font-semibold transition-all sm:px-6 sm:py-3.5 ${activeTab === 'skills'
              ...
              <button
                ...
                className={`group relative px-4 py-3 text-sm font-semibold transition-all sm:px-6 sm:py-3.5 ${activeTab === 'grammar'
                  ? 'text-black dark:text-white'
                  : 'text-gray-500 hover:text-gray-900 dark:text-gray-500 dark:hover:text-gray-300'
                  }`}
              >
              ...
              <button
                ...
                className={`group relative px-4 py-3 text-sm font-semibold transition-all sm:px-6 sm:py-3.5 ${activeTab === 'vocabularies'
                  ? 'text-black dark:text-white'
                  : 'text-gray-500 hover:text-gray-900 dark:text-gray-500 dark:hover:text-gray-300'
                  }`}
              >
              ...
              <button
                ...
                className={`group relative px-4 py-3 text-sm font-semibold transition-all sm:px-6 sm:py-3.5 ${activeTab === 'categories'
                  ? 'text-black dark:text-white'
                  : 'text-gray-500 hover:text-gray-900 dark:text-gray-500 dark:hover:text-gray-300'
                  }`}
              >
          ...
          <div className="flex-1 overflow-y-auto bg-gradient-to-b from-white to-gray-50 p-4 pb-16 dark:from-black dark:to-zinc-950 sm:p-6 lg:p-8">
            {activeTab === 'vocabularies' ? (
                        ...
                        return (
                          <div key={vocab.id} className="flex items-center gap-2">
                            <button
                              ...
                              className={`group w-full rounded-xl px-4 py-3 text-left text-sm font-medium transition-all ${isSelected
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold tracking-tight text-black dark:text-white">Category Main</h3>
                    <button
                  ...
                  {isAddingCategory && (
                    <div className="rounded-2xl border-2 border-black bg-white p-5 dark:border-white dark:bg-black">
                      ...
                      <input
                        ...
                        placeholder="Ej: Technology, Business..."
                        className="mb-3 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-black placeholder:text-gray-400 focus:border-black focus:outline-none focus:ring-2 focus:ring-black/10 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white dark:placeholder:text-zinc-600 dark:focus:border-white dark:focus:ring-white/10"
                  ...
                  <div className="space-y-2 rounded-2xl border border-gray-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
                    {categories
                      .filter(category => category.value.toLowerCase().includes(searchCategory.toLowerCase()))
                      .map(category => {
                          ...
                          <div key={category.id} className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => {
                                ...
                                setEditingCategoryOriginalValue(category.value);
                              }}
                              className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-wide text-gray-600 transition-all hover:border-black hover:bg-black hover:text-white dark:border-zinc-700 dark:bg-zinc-900 dark:text-gray-300 dark:hover:border-white dark:hover:bg-white dark:hover:text-black"
                            >
                              Editar
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold tracking-tight text-black dark:text-white">Sub Categories</h3>
                    <button
                  ...
                  {isAddingSubCategory && (
                    <div className="rounded-2xl border-2 border-black bg-white p-5 dark:border-white dark:bg-black">
                      <label className="mb-3 block text-sm font-semibold text-black dark:text-white">
                        Nueva Sub Category para: <span className="underline decoration-2 underline-offset-2">{selectedCategory}</span>
                      </label>
                      <input
                        type="text"
                    ...
                    {filteredSubCategories.map(subCategory => {
                      ...
                      return (
                        <div key={subCategory.id} className="flex items-center gap-2">
                          <div className="flex-1 rounded-xl bg-gray-50 px-4 py-3 text-sm font-medium text-gray-900 dark:bg-zinc-900 dark:text-gray-100">
                            {subCategory.value}
                          </div>
                          <button
                            onClick={() => {
                              setEditingSubCategoryId(subCategory.id);
                              setEditingSubCategoryValue(subCategory.value);
                            }}
                            className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-wide text-gray-600 transition-all hover:border-black hover:bg-black hover:text-white dark:border-zinc-700 dark:bg-zinc-900 dark:text-gray-300 dark:hover:border-white dark:hover:bg-white dark:hover:text-black"
                          >
                            Editar
                          ...
                          <button
                            onClick={() => handleDeleteSubCategory(subCategory.id, subCategory.value)}
                            className="rounded-lg border border-red-200 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-wide text-red-600 transition-all hover:border-red-600 hover:bg-red-50 hover:text-red-700 dark:border-red-900 dark:bg-zinc-900 dark:text-red-300 dark:hover:border-red-500 dark:hover:bg-red-950"
                          >
                            Eliminar
                          </button>
                          ...
                          return (
                            ...
                            >
                              <button
                                ...
                                className={`flex-1 text-left font-medium transition ${isSelected
